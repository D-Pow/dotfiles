...
buildAndDeploySelfServiceWebApp() (
    ...
    mvn clean install -Dmaven.test.skip=true -DskipTests -Djacoco.skip=true --projects ':engage-client'
    echo -e '\n\n'
    ( mvn clean install -Dmaven.test.skip=true -DskipTests -Djacoco.skip=true --projects ':SelfServiceWebApp'; )

    if (( $? )); then
        echo -e "\n\nError installing SelfServiceWebApp module. Re-attempting with building sub-dependencies as needed..."

        mvn install -Dmaven.test.skip=true -DskipTests -Djacoco.skip=true --projects ':SelfServiceWebApp,!:JettyServer,!:JettyServerJar' -am
    fi
